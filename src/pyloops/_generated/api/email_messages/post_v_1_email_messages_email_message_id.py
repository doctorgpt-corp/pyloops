...
from ...models.update_email_message_request import UpdateEmailMessageRequest
...
def _get_kwargs(
    ...
    *,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/v1/email-messages/{email_message_id}".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 413:
        response_413 = EmailMessageFailureResponse.from_dict(response.json())

        return response_413

    if response.status_code == 422:
        response_422 = EmailMessageFailureResponse.from_dict(response.json())
...
def sync_detailed(
    ...
    *,
    ...
    """Update an email message
     ...
     Update fields on an email message (subject, preview text, sender, LMX content). The campaign must be
    in draft status. Supply `expectedRevisionId` matching the current `contentRevisionId` — the server
    rejects mismatched revisions with 409.
